...
impl Query for UpdateEvent {
    ...
    async fn exec(&self, conn: &sqlx::PgPool) -> Result<Self::Success, Self::Failure> {
        ...
        if let Some(creator_id) = self.creator_id {
            pairs.push("creator_id = ");
            pairs.push_bind_unseparated(creator_id);
        }
        ...
        pairs.push("updated_at = CURRENT_TIMESTAMP WHERE id = ");
        pairs.push_bind_unseparated(self.event_id);
...
mod tests {
    ...
    async fn touch_record(pool: PgPool) -> Result<(), sqlx::Error> {
        let update = UpdateEvent::builder().event_id(73).build();
    ...
    async fn update_everything(pool: PgPool) -> Result<(), sqlx::Error> {
        let event = UpdateEvent::builder()
            ...
            .unwrap();
        assert_eq!(event.id, EventId(73));
        assert_eq!(event.creator_id, UserId(7));
        assert!(event.updated_at > event.created_at);
    ...
    #[sqlx::test(fixtures("./fixtures/jdongs-event.sql"))]
    async fn wrong_id_errors(pool: PgPool) -> Result<(), sqlx::Error> {
        let update = UpdateEvent::builder().event_id(17).build();
        let result = update.exec(&pool).await?;
        assert!(result.is_none());
        Ok(())
    }

    #[sqlx::test(fixtures("./fixtures/jdongs-event.sql"))]
    async fn missing_creator_fails(pool: PgPool) -> Result<(), sqlx::Error> {
        let update = UpdateEvent::builder().event_id(73).creator_id(99).build();
        let error = update.exec(&pool).await.expect_err("missing creator");
